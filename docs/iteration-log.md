...
## PX-43 — Hiring fit (2026-09-22, 0118–0122)

Timothy brought a brief on his résumé and portfolio: no first-round interviews, and materials that make a reviewer work to establish his role. He asked for more than one perspective. Astra's notes, an Opus agent's independent review and the session assistant's own read are filed verbatim, with his answers, beside the résumé source outside this repository (0115 keeps résumé and LinkedIn matters private).

**Résumé.** Rebuilt privately from one source outside this repository, two tracks with the same facts, and the cover letter joined the same build. Only the phone-less copy the site links to is in `static/resume/`.

**Site.** Home: a role line and a status line with the résumé link under “i'm tim.”, and the hint reads “Selected work” and links to the cards; the chips read Brand, Motion, Web; the statement keeps “Designer for teams that don't have one yet.” with a companion sentence. Jade Aesthetics replaces First Ledger in the selected four (his pick); First Ledger is index 05 and the numbers run to 09. Every study opens with one contribution-first lead paragraph; PARC's origin story is its first block and its decision paragraph is his own account (an earlier direction the team did not take, then the finished logo on its sign). Study paragraphs open to 27/40 in a 64ch column, still on the unit; no new face. The lead was first set at 54/72, then on Timothy's review the same evening (“their text is really big for the body copy”) dropped to body size, 27/40 in a 64ch column, at every width (0123). A Team row appears where the data names collaborators (PARC, Pocketwatch). Contact says hiring or project, states full-time and freelance availability, and links the résumé, the email and LinkedIn. Page titles use the slash of the chrome, quote attributions follow a slash, two Jade headings take a colon; the copy carries no em dashes. The label reads “Resume”: PARC Pixel has no É and the fallback glyph showed. Share images regenerated for the new numbers and titles.

**Checks.** `pnpm check` 0 errors 0 warnings; 24 unit tests; production build; `social:verify` on 18 pages; `tools/review/blocks.mjs` 0 block tops off the 8px unit on 28 route/width checks; both new hero links take the accent focus ring; the hero reads on the light theme; the “Selected work” link lands on the work section. The two console 403s in the static preview are the analytics function, which only runs on Vercel. Evidence: `docs/iterations/pixel-v2/43-hiring-fit/` (before from the live site, after from the preview, both at 1440 and 390). Timothy reviewed the build on the preview the same evening and asked for everything to be committed and pushed. Pushed as `22f1eb7` with PX-44; Vercel served it at 16:51 MT: the new title, the hosted résumé (200), the PARC wide logo and the regenerated share images checked on the live origin.

Outside the repo, the same day: on Timothy's instruction his LinkedIn profile was brought in line with the site's positioning; the record is private, beside the résumé source. Still open here: the PARC earlier-direction artwork can join the study when he sends it.


PX-43 asset correction, same evening: Timothy saw the PARC before/after row's right-hand image showing sky only. The file was the club's X header, which is sky by design (the avatar carries the logo; the brand folder's own copy is named “minimal”), so the slot had never held a lockup and its alt text was wrong since the PX-08 port. It is replaced by `mark-wide.png`, a 3:1 crop of the wide logo on the sky taken from the Discord banner in the brand folder, so the row now pairs the old mascot and wordmark with the new stacked and wide marks. `twitter-banner.png` is removed.

## PX-44 / Specific copy, shorter application materials (2026-09-22, 0124)

Timothy accepted the full copy review and replacement drafts. Home's new copy names identities, websites, and motion and explicitly welcomes an existing design team. Jade's opening is repaired and its repeated closing sections merged; PARC and xrp.cafe get shorter outcomes; Sonde's long backend list becomes one reading-focused paragraph. Contact uses Your message and Tell me about the role or project. Toolbox's download heading is explicit; a 320px overflow found during review was fixed with Jacquard 43/48 below 420px. The rest of the accepted typography and Timothy's newer study lead are preserved.

Outside the public repo, résumé summaries and the long brand bullets were shortened, project links added, and web-track platform figures separated from contributions. Both cover letters use the accepted concrete project examples, track-specific openings, and shorter closing. The builder can insert a supplied employer-fit sentence. Personal facts and platform-scale sentences were verified unchanged against the archived source. All five PDFs remain one page with selectable text and embedded fonts; each fresh render was inspected. The phone-free public copy was synced and served bytes verified. Private source/PDF baselines and verification are retained beside the editable sources.

Svelte check 0 errors/warnings, production build and 18-page metadata checks pass. Browser checks cover six edited routes at 1440/390/320 with no horizontal overflow, plus contact validation without sending a message. A fresh preview on port 4175 resolves the older preview processes' stale module references. Source before/after, the isolated diff, and verification are in `docs/iterations/pixel-v2/44-copy-edit/`. Implementation authorized; not committed, pushed, or deployed. The two project-evidence gaps from the review remain documented rather than filled speculatively.

## PX-45 — The PARC first direction (2026-09-22, 0125)

The gap the three reviews all pointed at, between “the team was worried” and “the comments were encouraging”, now has its middle. Timothy sent the direction the team turned down: three branch lockups, The Nightclub, The Parcade and The Rowboat Club, black pixel wordmarks with a red icon each. The PARC study shows the sheet under “An earlier direction”, dated around 2024 on his correction, with one paragraph in his account, before the shipped logo. Rendered from his SVG through the site's Chromium onto white, 1600×1000 plus a 2× JPEG, noted “Set aside, around 2024”. Pushed with PX-46 on his word.

## PX-46 — One PARC study (2026-09-22, 0126)

Timothy asked for the PARC brand and PARC Website studies to be one. The brand study keeps its arc and gains the website as a second act, in the website study's own words, with about half of its screenshots; the brand study's duplicate site shots go. One reaction list and one outcome list close it. The old URL redirects. The share-image generator now takes `OG_PORT` from the environment, because 4174 was Timothy's own preview and 4175 Codex's. Check, build and the 17-page metadata verification pass. Reviewed on the preview and pushed the same evening as `58cf91b`; Vercel served it at 17:22 MT, the merged study, the eight-entry index and the 308 redirect from the old URL all checked on the live origin.

## PX-47 — The hero on a phone (2026-09-22, 0127)

Timothy's phone screenshot of the live PX-43 hero: four label lines under the name, a slash left hanging, the résumé link indented on its own line, the work chip under the fold, and the strip's path clipped on a 430px phone. The hero now fills the first small-viewport screen on phones, the status stacks cleanly at 480px and below, and the résumé link sits beside the “Selected work” chip in one actions row (bottom right on desktop). The clock sheds at 480px. Before and after first-screen captures at 375, 390 and 430 in `docs/iterations/pixel-v2/47-phone-hero/`.

## PX-48 — The hero is the terminal (2026-09-22, 0128)

PX-47's phone hero still stacked too many static pieces under the name. On Timothy's direction the copy joins the typing the hero already does: “hi.” then “i’m tim.” in the name slot, then the three copy lines one at a time under it behind the cell cursor, a hold, a clear, and round again, about eighteen seconds a loop, paused off screen. Motion off and the prerender show the name and the copy in full. Frames of the loop at 430px, the motion-off rendering and a desktop frame are in `docs/iterations/pixel-v2/48-terminal-hero/`.

Timothy's first look: “hi and im tim dont transition properly. im tim doesnt get typed out like it should.” The cause was in `Decode`, older than the loop: its start-when-visible effect called the typing routine directly, and that routine reads the text prop, so every text change started two runs at once (the readout's path and the study titles had the same double run, unnoticed in scramble mode). The two intervals typed over each other and a finished one kept writing the whole text back. `Decode` now cancels the pending start and the running interval before a new run, an interval only clears itself, and the visibility effect reads the props inside `untrack`. Verified by recording every change of both slots for 22 seconds: 152 changes, each a single character or a clear. Pushed as `85c1720`; Vercel served it within minutes, and the live name slot recorded from a phone-sized browser typed “i’m tim.” one character at a time after “hi.”.
