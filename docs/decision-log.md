...
## 0127 — The hero on a phone

- **Date:** 2026-09-22
- **Direction:** Timothy, from his phone on the live site: “This new hero doesn’t read great on mobile.” His screenshot (a 430px phone) showed the status wrapped to three label lines with a stray slash and an indented résumé link, the “Selected work” chip below the fold, and the strip's path clipped to “~/T…”.
- **Implementation:** On phones the hero is the small viewport less the 136px chrome (`100svh`, 80vh as the fallback), so the name, the role line, the status and the actions end at the fold. The résumé link leaves the status line and joins the “Selected work” chip in one actions row: under the text on phones, bottom right on desktop with the chip outermost. The status keeps two segments; at 480px and below (the one-line form needs about 464px) they stack as two lines and the slash between them is hidden. The strip sheds the clock at 480px instead of 420, so 430px phones keep their path.
- **Status:** Timothy's direction; the composition is the assistant's. Superseded the same evening by 0128, which keeps its hero height, actions row and clock breakpoint; pushed with it.

## 0128 — The hero is the terminal

- **Date:** 2026-09-22
- **Direction:** After the PX-47 rework, Timothy: “its still feels busier than i want it to be. it works on desktop because it doesnt take up a huge percentage of the screen. maybe the hero becomes a looping animation. hi. im tim. then the rest of the copy. then loop? so it feels more integrated instead of tacked on?” and then “basically it all gets integrated into the terminal style hero animation.”
- **Implementation:** With motion on, the name slot types “hi.”, holds, then “i’m tim.”; the line under it types the copy one line at a time behind the accent cell cursor (“Brand and web designer.”, “Identities, motion, and websites built in code.”, “Denver / remote. Open to full-time and freelance.”), holds, then both slots clear and the loop restarts. Only one cursor shows at a time: the name's em cursor until a line is typing, then the line's. The loop pauses while the hero is off screen. The label-face status line is gone on every width; the actions row (the “Selected work” chip and the résumé link) stays. Motion off, no JavaScript and the prerender show “i’m tim.” and the copy as one static paragraph, which is also what assistive technology reads (the animated slots are aria-hidden behind sr-only copies). `Decode` gained a `cursorSize` prop for the cell cursor. The typed line sits in a fixed two-line flex box so the field's knockout stays put as the words change.
- **Status:** Timothy's direction; the sequence, timings and copy lines are the assistant's. Reviewed on the preview, the typing fix verified, and pushed the same evening.
