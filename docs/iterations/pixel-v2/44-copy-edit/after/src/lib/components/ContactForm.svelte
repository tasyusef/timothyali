<script lang="ts">
  ...
  const NEED: Record<string, (el: HTMLInputElement | HTMLTextAreaElement) => string> = {
    ...
    message: (el) => (el.value.trim() ? '' : 'Tell me about the role or project.')
  };
...
<form id="contact-form" class="contact-form" action="/api/contact" method="post" novalidate onsubmit={submit} aria-describedby="contact-status">
  ...
  <label class="field"><span class="lbl dim">Your message</span><span class="box"><textarea class="body" name="message" aria-invalid={notes.message ? 'true' : undefined} aria-describedby={notes.message ? 'message-note' : undefined} oninput={clear} rows="4" required maxlength="5000" use:blockCaret></textarea><span class="cursor caret" aria-hidden="true"></span></span>{#if notes.message}<span class="lbl note" id="message-note">{notes.message}</span>{/if}</label>
