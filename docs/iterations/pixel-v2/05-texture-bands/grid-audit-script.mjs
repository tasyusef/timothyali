...
const near=(a,b)=>Math.abs(a[0]-b[0])<12&&Math.abs(a[1]-b[1])<12&&Math.abs(a[2]-b[2])<12; const Y=[242,214,0],K=[17,17,14];
for(const width of [1440,390]){ for(const path of ['/','/work/','/contact/']){
  ...
  const r=await p.evaluate(()=>{
    ...
    const walker=document.createTreeWalker(document.body,NodeFilter.SHOW_ELEMENT); let n; const table={'Jacquard 24':43,'Jersey 25':41,'Jersey 15':27,'Silkscreen':8,'Press Start 2P':8};
  ...
  if(path==='/'){ // solid text crispness
    const rep={}; for(const [k,sel] of Object.entries({hero:'.hero-name',statement:'.statement h2',body:'.statement .body',quality:'.quality h2',cards:'.card h3',invite:'.invitation h2'})){ const el=await p.$(sel); if(!el)continue; await el.scrollIntoViewIfNeeded(); await p.waitForTimeout(150); const png=PNG.sync.read(await el.screenshot()); let mid=0,tot=0; for(let i=0;i<png.data.length;i+=4){const px=[png.data[i],png.data[i+1],png.data[i+2]];tot++;if(!near(px,Y)&&!near(px,K))mid++;} rep[k]=+(100*mid/tot).toFixed(2);} console.log('intermediate px % (0 = crisp):',JSON.stringify(rep)); }
  await ctx.close(); }}
await b.close();
