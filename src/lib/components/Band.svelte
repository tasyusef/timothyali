<script lang="ts">
  ...
  // shape the knockout around that text, in cells; `flip` mirrors the field. All three
  // are forwarded as given, so Ascii's defaults are the only defaults.
  import type { Snippet } from 'svelte';
  import Ascii from './Ascii.svelte';
  import { TICK_SLOW } from '$lib/tokens';
  type Mode = 'noise' | 'fall' | 'scan' | 'sparse' | 'sky' | 'bands';
  let { as = 'section', class: klass = '', mode = 'noise', seed = 7, tick = TICK_SLOW, density = 1, avoid = '', pad, feather, flip, children, ...rest }:
