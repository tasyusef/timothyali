...
- `slug`, `title`, `section` (`'passion' | 'work'`), `category`, `year`, `heroImage`, `heroAspect`
...
The homepage renders two grouped work sections — "Passion Projects & Open Source" (`passionProjects`: self-initiated apps and open source — Pocketwatch, Sonde; a new Gridform app will join here) then "Selected Work" (`workProjects`: client/professional and studio work), both derived from `section`. Desktop shows `ProjectBento` — aspect-justified bento bands: consecutive projects sharing a `bentoRow` number form one band; within a band, consecutive projects sharing a `bentoCol` stack vertically in one column. Current curation: passion band = Pocketwatch (landscape ad) + Sonde; work bands = FirstStrike + xrp.cafe, Jade + First Ledger, Do Androids Dream + Studio Gridform. A solver sizes columns so every column of a band is equal height, and stacked cards split their column via flex-grow ∝ 1/aspect — all media renders aspect-true with zero cropping (same technique as the Pocketwatch case-study hero pair). Cards are full-bleed media tiles (no panel/radius); the meta chin (number, title, category, year) overlays the bottom edge and appears only on hover/focus. `bentoImage`/`bentoAspect` override the card media when the hero doesn't suit a tile (Pocketwatch uses its landscape ad). Mobile shows `ProjectIndex` (tap-to-expand list). The array is ordered passion-first so numbering runs continuously (01–02, then 03–08 via the shared `startIndex` prop) and `getNextProject` cycles passion → work. New projects must set `section` (and optionally `bento`).
