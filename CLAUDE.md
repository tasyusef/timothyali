...
8. Case-study-local interactive demos colocate in the route folder (non-`+` files are private modules); promote to `$lib/components` only when shared. Demo content colors must be the showcased product's real values, clearly commented as product colors, never site tokens. The one embedded demo today is the PARC brand system (`src/routes/work/parc/BrandSystem.svelte` + `BrandRule`, `ParcMark`, `LogoStory` (before/after of the marks), `Quotes` (verbatim community reaction), `World` (the four collections as animated WebPs from `static/parc`, plain `<img>` on purpose, plus `IslandScene`, each on a `PxCard`), `LivingTexture`, `SignalTexture`, `ChatTexture`, `SkyClouds`, `IslandScene` + `bankAtlas.ts` (Rowboat Racer's island drawn from the game's sprite sheet in `static/parc/game`), `parcBrand.ts` (the textures and the island are ports of the PARC site's generated backgrounds)): the brand sheet rebuilt live, with PARC Pixel woff2s and the cloud tile served from `static/parc/` and `@font-face` declared inside the component. It replaced a 1280×4800 screenshot of the sheet. Site captures in `src/lib/images/parc/site-*.png` / `phone-*.png` were shot from the live parcxrpl.com over the DevTools protocol; the site sends `frame-ancestors 'none'`, so it cannot be a `LiveEmbed`. (A Pocketwatch category-pill demo was tried and scrapped.)

### Brand-Themed Case Studies
Each case study is styled after the brand it is about. The mechanism is small and shared; PARC (`/work/parc`) is the reference implementation, and the PARC website case study (`/work/parc-site`) shares it: its `+page.ts` also returns `brand: 'parc'` and it imports the theme stylesheet and the route-local components (`SkyGround`, `HangingSign`, `PxButton`, `PxCard`, `ParcBand`, `Quotes` with its `title`/`quotes` props) from `../parc/`. Its captures in `src/lib/images/parc-site/` were shot from the live site on Sep 7, 2026 at 1440 and 390 wide; the four `game-*.png` files are the Rowboat Racer art kept when that game's standalone case study was retired and folded into the PARCade section (`vercel.json` 308-redirects `/work/rowboat-racer` → `/work/parc-site`).
1. **The route declares its brand.** `src/routes/work/<slug>/+page.ts` exports `load = () => ({ brand: '<slug>' })`.
2. **The root layout stamps it.** `+layout.svelte` wraps skip link, nav, main, footer, and command menu in `<div data-brand={page.data.brand}>` (attribute absent elsewhere).
3. **Brand CSS lives with the route** (`src/routes/work/<slug>/<slug>-theme.css`, imported by that `+page.svelte`) and scopes every rule under `[data-brand='<slug>']`. It re-points the site tokens (`--color-*`, `--font-*`, `--radius-panel`, `--ease-swiss`) at the brand, so shared components take the look without forks. Because the wrapper sits under `<html data-theme>`, a brand page ignores the dark/light toggle by design (hide `.theme-toggle` there). Brand CSS may use the product's real colors; shared components never do.
4. **Stable hooks on shared components** are what brand CSS targets: `.site-nav`, `.site-footer`, `.site-status`, `.theme-toggle`; on `CaseStudy`: `.cs`, `.cs-header`, `.cs-back`, `.cs-title`, `.cs-overview`, `.cs-meta`, `.cs-hero`, `.cs-body`, `.cs-next`; `.cs-text` (+ `.copy`) on `TextSection`, `.cs-results` on `ResultsList`, `.cs-gallery` on `Gallery`. Classes only; unbranded pages look the same.
5. **One markup escape hatch.** `CaseStudy` accepts an optional `heading` snippet that replaces the default `<h1>` block (PARC's hanging sign). Everything else is CSS plus route-local components.
PARC's theme: the whole page lives on the site's sky, fixed behind everything with the two drifting cloud layers (`SkyGround`; `.cs`, nav, and footer sit above it), green nav and footer, PARC Pixel for headings/labels/data with Space Grotesk for prose (PARC's own type rule), and everything else on paper: the notched `--pix` panel with a green-dark slab for overview, text sections, the metadata card (`.cs-meta-list`), and the brand sheet (`.parc-sheet`); small paper tags for section eyebrows (`.cs-text h2`, `.cs-results h2`, `.parc-tag`); `PxCard` (port of the site's CollectionCard, slab in one of the four colors, `--card` = its ink color) for the collections, the island, the before/after marks, and the quotes; gallery thumbnails framed as one-cell paper cards; `HangingSign` title, `PxButton` links, `ParcBand` for the closing green Outcome band that runs into the footer, and `steps()` easing. Gallery masonry is round-robin over four columns, so order items with column height in mind (the two phone captures share the right-hand columns).
...
The homepage renders three grouped work sections, all derived from `section`: "Brand & Art Direction" (`brandProjects`: FirstStrike, PARC, xrp.cafe, First Ledger, Studio Gridform), then "Web & Product Design" (`webProjects`: website/product work — Jade Aesthetics, PARC Website), then "Passion Projects, School Work & Open Source" (`passionProjects`: self-initiated apps, school work, and open source — Pocketwatch, Sonde, GRIDFORM Studio, Do Androids Dream). The array is ordered brand → web → passion so numbering runs continuously (01–05, 06–07, 08–11 via the shared `startIndex` prop) and `getNextProject` cycles in the same order. New projects must set `section`.
...
└── routes/
    ...
    ├── work/                   # One folder per case study (11 total)
    │   └── <slug>/+page.svelte
