...
PARC's theme: white ground with the site's hero scatter fixed behind the whole page (`ScatterGround`; `.cs` lifts content above it and lets pointer events fall through to bare ground), green nav and footer, PARC Pixel for headings/labels/data with Space Grotesk for prose (PARC's own type rule), paper panels with the notched `--pix` polygon for overview and text sections, `HangingSign` title, `PxButton` links, `ParcBand` for the sky (World) and green (Reaction, Outcome) bands, and `steps()` easing.
