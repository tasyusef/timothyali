...
// usage: node tools/social/story-video.mjs [seconds] [story|wide] — story is 1080×1920 for
// Instagram, wide is 1920×1080 for Twitter/X; both come from /og/<id>.
const FPS = 30, SECONDS = Number(process.argv[2] ?? 8), FRAMES = FPS * SECONDS;
const ID = process.argv[3] ?? 'story';
const [W, H] = ID === 'wide' ? [1920, 1080] : [1080, 1920];
const OUT = `docs/social/${ID}-launch.mp4`;
...
for (let i = 0; ; i++) { try { if ((await fetch(`${ORIGIN}/og/${ID}`)).ok) break; } catch {} if (i > 200) { stop(); throw new Error('dev server did not start'); } await new Promise((r) => setTimeout(r, 100)); }
...
const dir = `docs/social/${ID}-frames`; rmSync(dir, { recursive: true, force: true }); mkdirSync(dir, { recursive: true });
for (let i = 0; i < FRAMES; i++) {
  ...
  await page.screenshot({ path: `${dir}/${String(i).padStart(4, '0')}.png`, clip: { x: 0, y: 0, width: W, height: H } });
}
await b.close(); stop();
execFileSync(ffmpeg, ['-y', '-framerate', String(FPS), '-i', `${dir}/%04d.png`, '-c:v', 'libx264', '-preset', 'slow', '-crf', '16', '-pix_fmt', 'yuv420p', '-movflags', '+faststart', OUT], { stdio: 'inherit' });
rmSync(dir, { recursive: true, force: true });
console.log(`${OUT}: ${SECONDS}s at ${FPS}fps, ${FRAMES} frames`);
